...
func generateApiMain(cobraCmd *cobra.Command, args []string) {
	prefixFilter := AcwConfig.ApiPrefixFilter
	...
	AcwConfig.ApiPrefix = generateApiPrefix(allApis)

	apiOptions := map[string]config.AcwConfigApiOpt{}
	flags := cobraCmd.Flags()
	generateApiParameters(apiOptions, allApis, flags)

	AcwConfig.ApiOptions = apiOptions
...
func generateApiParameters(apiOptions map[string]config.AcwConfigApiOpt, allApis []string, flags *flag.FlagSet) {
	flags.Bool(CMD_SHOW_HELP, true, "")

	for _, api := range allApis {
		var opt *config.AcwConfigApiOpt
...
func generateEc2ApiParameters(api string) *config.AcwConfigApiOpt {
	...
	oldOpts, ok := AcwConfig.ApiOptions[api]
	if ok {
...
func generateAdminVpcParameters(api string, flags *flag.FlagSet) *config.AcwConfigApiOpt {
	inCmds := []string{api}
